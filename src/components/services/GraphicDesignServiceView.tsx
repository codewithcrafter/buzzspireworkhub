      ...
      <section className="py-20 md:py-28 px-6 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          ...
          <ScrollReveal delay={0.2} className="relative h-[500px] hidden lg:block w-full">
            {/* Custom Graphic Design "Creative Canvas" Visual Composition */}
            <div className="absolute inset-0 bg-gradient-to-tr from-violet-500/5 via-transparent to-primary/5 rounded-[2.5rem] border border-border/50 overflow-hidden shadow-2xl flex items-center justify-center p-8 bg-[url('/noise.png')]">
               ...
               <div className="w-full h-full relative z-10">
                 ...
                 <div className="absolute top-4 left-4 bg-white dark:bg-zinc-900 border border-border rounded-xl p-3 shadow-xl flex flex-col gap-2 animate-float-slow z-30">
                   <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Brand Colors</div>
                   <div className="flex gap-2">
                     <div className="w-6 h-6 rounded-full bg-[#8B5CF6] shadow-sm"></div>
                     <div className="w-6 h-6 rounded-full bg-[#EC4899] shadow-sm"></div>
                     <div className="w-6 h-6 rounded-full bg-[#10B981] shadow-sm"></div>
                     <div className="w-6 h-6 rounded-full bg-[#F59E0B] shadow-sm"></div>
                   </div>
                 </div>

                 {/* Typography Node */}
                 <div className="absolute top-12 right-4 bg-white dark:bg-zinc-900 border border-border rounded-xl p-4 shadow-xl flex flex-col items-center justify-center animate-float-medium z-30">
                   <div className="font-serif text-4xl text-foreground leading-none">Aa</div>
                   <div className="text-[10px] text-muted-foreground mt-2 font-mono">Playfair Display</div>
                 </div>

                 {/* Vector Graphic UI */}
                 <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-zinc-900 rounded-2xl border border-border shadow-2xl w-[280px] p-2 z-20 overflow-hidden group">
                   <div className="flex gap-1.5 px-2 py-2 border-b border-border/50">
                     <div className="w-2.5 h-2.5 rounded-full bg-red-400"></div>
                     <div className="w-2.5 h-2.5 rounded-full bg-amber-400"></div>
                     <div className="w-2.5 h-2.5 rounded-full bg-green-400"></div>
                   </div>
                   <div className="p-4 relative">
                     {/* Vector Path Abstract */}
                     <div className="w-full aspect-[4/3] rounded-lg bg-muted relative overflow-hidden group-hover:scale-[1.02] transition-transform duration-500">
                       <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
                         <path d="M 0,100 C 20,80 40,10 60,40 C 80,70 90,20 100,0 L 100,100 Z" fill="url(#grad)" opacity="0.8" />
                         <path d="M 0,100 C 30,60 50,40 70,80 C 85,100 95,50 100,20" fill="none" stroke="currentColor" strokeWidth="2" className="text-primary" />
                         <defs>
                       ...
                       {/* Control Points */}
                       <div className="absolute top-[40%] left-[60%] w-2 h-2 bg-primary rounded-full ring-2 ring-white transform -translate-x-1/2 -translate-y-1/2 animate-pulse"></div>
                       <div className="absolute top-[80%] left-[70%] w-2 h-2 bg-primary rounded-full ring-2 ring-white transform -translate-x-1/2 -translate-y-1/2 animate-pulse" style={{animationDelay: '1s'}}></div>
                     </div>
                   </div>
                 </div>

                 {/* Layers Panel */}
                 <div className="absolute bottom-8 left-8 bg-white dark:bg-zinc-900 border border-border rounded-xl p-3 shadow-xl flex flex-col gap-2 animate-float-slow z-30" style={{animationDelay: '1.5s'}}>
                   <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Layers</div>
                   <div className="flex items-center gap-2 bg-muted/50 px-2 py-1 rounded text-xs font-medium text-foreground"><div className="w-2 h-2 rounded-full bg-primary"></div> Vector Smart Object</div>
                   <div className="flex items-center gap-2 px-2 py-1 rounded text-xs text-muted-foreground"><div className="w-2 h-2 rounded-full border border-muted-foreground"></div> Background Copy</div>
                   <div className="flex items-center gap-2 px-2 py-1 rounded text-xs text-muted-foreground"><div className="w-2 h-2 rounded-full border border-muted-foreground"></div> Background</div>
                 </div>
