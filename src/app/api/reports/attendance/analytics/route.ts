...
export async function GET(req: Request) {
  try {
    ...
    if (!hasPermission(auth.role, "REPORT_READ_ALL") && auth.role !== "EMPLOYEE") {
      return ApiResponse.forbidden("Access denied: Permission required", req);
    }
    ...
    const analytics = await ReportService.getAttendanceAnalytics({
      startDate,
      endDate,
      employeeId,
    ...
    return ApiResponse.serverError("Error fetching attendance analytics", error, req);
  }
}
