...
export default function EmployeeReportsPage() {
  ...
  const hasAccess = profile?.role === "ADMIN" || profile?.permissions?.includes("REPORTS_VIEW");
    ...
    return (
      <Card className="rounded-3xl border border-destructive/30 bg-destructive/5 p-8 text-center space-y-3">
        ...
        <p className="text-sm text-muted-foreground max-w-md mx-auto">
          You do not have permission to view reports (<code>REPORTS_VIEW</code>). Please contact your administrator.
        </p>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Analytics on deal cycles, conversion velocity, and campaign reach.
        </p>
      </div>
      ...
      <Card className="rounded-2xl border border-border/80 shadow-sm p-12 text-center space-y-2 bg-card">
        ...
        <p className="font-bold text-foreground text-base">Analytics Reports Workspace</p>
        <p className="text-xs text-muted-foreground max-w-md mx-auto">
          You are authorized with <code>REPORTS_VIEW</code>. Operational analytics and conversion funnel statistics will be charted here.
        </p>
      </Card>
    </div>
  );
